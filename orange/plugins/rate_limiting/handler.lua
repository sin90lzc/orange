        ...
        if rule.enable == true then
		    ...
		    if currentValue >= handle.warncount then
                        if handle.log == true then
                            ngx.log(ngx.WARN, "[RateLimiting-Warn-Rule]", rule.name, " uri:", ngx_var_uri," warn-limit:",handle.warncount, " limit:", handle.count, " reached:", currentValue , " remaining:", handle.count-currentValue)
                        end
		    end
                    if handle.count > 0 and currentValue >= handle.count then
                        if handle.log == true then
                            ngx.log(ngx.WARN, "[RateLimiting-Forbidden-Rule] ", rule.name, " uri:", ngx_var_uri, " limit:", handle.count, " reached:", current_stat, " remaining:", 0)
                        end
                        ngx.exit(429)
                        return true
                    end
